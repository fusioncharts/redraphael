...
var ef = R.easing_formulas = {
    ...
    elastic: function (n) {
        ...
        return pow(2, -10 * n) * mathSin((n - 0.075) * (2 * PI) / 0.3) + 1
    },
    bounce: function (n) {
        var s = 7.5625,
            p = 2.75,
 ...
 *
 * @param {object} el  element to sync with
 * @param {object} anim  animation to sync with
 * @param {object} params  #optional final attributes for the element, see also @Element.attr
 * @param {number} ms  #optional number of milliseconds for animation to run
 * @param {string} easing  #optional easing type. Accept on of @Raphael.easing_formulas or CSS format: `cubic&#x2010;bezier(XX,&#160;XX,&#160;XX,&#160;XX)`
 * @param {function} callback  #optional callback function. Will be called at the end of animation.
 * @param {object} configObject  #optional takes an object with optional properties like:
    - start - what percentage to start animation with respect to parent animating element
    - end - what percentage to end animation with respect to parent animating element
    - hookFn - function to be called before starting animation
    - smartMorph - whether to use smartMorphing in path animation
 * or
 * @param {object} element  element to sync with
 * @param {object} anim  animation to sync with
 * @param {object} animation  #optional animation object, see @Raphael.animation
 **
...
/**
 * Function to convert two color string and return an array containing two colors
 * in such a way that they can be animated.
 * Currently the function supports
 * - Linear Gradient - Linear Gradient,
 * - Solid Color - Solid Color
 * - Solid Color - Linear Gradient & viceversa
 * - Solid - Radial
 * - Radial - Radial
 *
 * To know proper color format refer RedRahpael docs
 *
 * @example
 *  colorNormalizer("270-rgba(69,144,135,1)-rgba(255,255,255,1)",
 *      "270-rgba(69,144,151,1)-rgba(255,255,255,1)", getRgbFn)
 *
 *
 *  Output
 *
 ...
 * @param {String} c1
 * @param {String} c2
 * @param {Function} getRGB
 * @returns {Array} Animatable initial and final state paths
 */
...
/**
 * Function to take two path array and return an array containing two paths,
 * that can be easily animated.
 * @example
 * p1 = [
 *     [M,88.5,299],
 *     ['C',559,299,559,299,559,299],
 *     ['C',559,299,559,299,559,299"]..
 * ]
 * 
 * p2 = [   
 *     [M,87.5,299],
 *     ['C',87.5,299,558,299,558,299],
 *     ['C',558,68.6,558,299,558,299"]...
 * ]
 * 
 * pathNormalizer(p1, p2);
 *
 *
 * Output
 *
 ...
 *     ]
 * ]
 * @param {Array} p1 - Initial path Array
 * @param {Array} p2 - Final path Array
 * @returns {Array} Animatable initial and final state colors
 */
